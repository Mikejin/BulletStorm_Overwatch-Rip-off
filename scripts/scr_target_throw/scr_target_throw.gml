...
if (instance_number(e) > 0) {                       // Make sure there are atleast enemies on the screen
    ...
    for (var i = 0; i < array_length_1d(enemy); i++) {
        ...
        if (distance <= throwRange) and currentinst.team != team
    ...
    throwTarget = ds_list_find_value(inrange, n - 1);                        // Set the target to the first enemy in the list plus the "n" Value
    if (is_undefined(throwTarget)) throwTarget = noone;                              // If there is no enemy at that position, set the target to our own ID
    ds_list_destroy(inrange);                                           // Destroy the "inrange" DS List
    return throwTarget;                                                      // Return the Target
} else {
    throwTarget = noone;                // Set the target to our ID because there are no enemies in range
    ds_list_destroy(inrange);   // Destroy the "inrange" DS List
    return throwTarget;              // Return the target
}
