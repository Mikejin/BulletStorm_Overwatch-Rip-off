/// @description  scr_target_weakest(target,n)
/// @function  scr_target_weakest
/// @param target
/// @param n
...
weakest = ds_list_create(); // Create the "weakest" Ds List
...
if (instance_number(e) > 0) {                       // Make sure there are atleast enemies on the screen
    ...
    ds_list_sort(hp_list, true);                                            // Sort the DS list in acending order
    ...
    for (var i = 0; i < array_length_1d(enemy); i++) {
        for (var j = 0; j < array_length_1d(enemy) j++) {
            ...
            if (ds_list_find_value(hp_list, i) == hp) {                     // Check the "hp_list" DS list for the distance of the current enemy
                ds_list_add(weakest, currentinst);                          // If it matches then add the current enemy into the weakest array
            }
        }
    }
    ...
    target = ds_list_find_value(weakest, n - 1);                            // Set the target to the weakest enemy plus "n"
    ...
    ds_list_destroy(weakest);                                               // Destroy the "weakest" DS List
