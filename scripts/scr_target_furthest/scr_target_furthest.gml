/// @description  scr_target_furthest(target,n)
/// @function  scr_target_furthest
/// @param target
/// @param n
...
furthest = ds_list_create(); // Create the "furthest" Ds List
...
if (instance_number(e) > 0) {                       // Make sure there are atleast enemies on the screen
    ...
    ds_list_sort(dist, false);                                               // Sort the DS list in decending order
    ...
    for (var i = 0; i < array_length_1d(enemy); i++) {
        for (var j = 0; j < array_length_1d(enemy) j++) {
            ...
            if (ds_list_find_value(dist, i) == distance) {                  // Check the "dist" DS list for the distance of the current enemy
                ds_list_add(furthest, currentinst);                         // If it matches then add the current enemy into the furthest array
            }
        }
    }
    ...
    target = ds_list_find_value(furthest, n - 1);                           // Set the target to the furthest enemy plus "n"
    ...
    ds_list_destroy(furthest);                                              // Destroy the "furthest" DS List
