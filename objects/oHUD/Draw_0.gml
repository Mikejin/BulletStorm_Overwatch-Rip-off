...
//SkillCD
	switch (belongsTo.heroNumber)
	{
		...
		case 3://莱因哈特
		draw_sprite_ext(sprDashBar,0,camX+camW/2-sprite_get_width(sprDashBar)/2,camY+camH-80,(belongsTo.shieldHP/belongsTo.shieldHPMax),1,0,c_white,0.8);
		draw_text_transformed(camX+camW/2-10,camY+camH-53,string(global.heroArray[belongsTo.heroNumber,4]),1,1,0)
		break;
		...
		{
		draw_sprite_ext(sprDashBar,0,camX+camW/2-sprite_get_width(sprDashBar)/2,camY+camH-80,(round(belongsTo.alarm[2]))/belongsTo.transformTime,1,0,c_white,0.8);
...
//弹药
	...
	draw_set_font(ft_overwatch);
	//最大弹夹数量
	draw_text_color(camX+camW/2-sprite_get_width(sprHB)/2-30,camY+camH-10,string(belongsTo.clipAmount),c_white,c_white,c_white,c_white,0.8)
