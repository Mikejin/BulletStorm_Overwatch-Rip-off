/// @description  scr_target_fastest(target,n);
/// @function  scr_target_fastest
/// @param target
/// @param n
...
fastest = ds_list_create(); // Create the "fastest" Ds List
...
if (instance_number(e) > 0) {                       // Make sure there are atleast enemies on the screen
    ...
    ds_list_sort(spd_list, false);                                          // Sort the DS list in decending order
    ...
    for (var i = 0; i < array_length_1d(enemy); i++) {
        for (var j = 0; j < array_length_1d(enemy) j++) {
            ...
            if (ds_list_find_value(spd_list, i) == spd) {                   // Check the "spd_list" DS list for the distance of the current enemy
                ds_list_add(fastest, currentinst);                          // If it matches then add the current enemy into the fastest array
            }
        }
    }
    ...
    target = ds_list_find_value(fastest, n - 1);                            // Set the target to the fastest enemy plus "n"
    ...
    ds_list_destroy(fastest);                                               // Destroy the "fastest" DS List
